#!/bin/bash
#
# Ray Cluster Head Node Setup Script
# For Intel i5-7500 4C/4T @ 3.8GHz with 24GB RAM
# -------------------------------------------
# This script sets up a machine as a Ray cluster head node with all necessary
# configurations for the AI assistant cluster infrastructure.
#
...
if [ "$SKIP_UPDATE" = false ]; then
    ...
    apt update || { error "Failed to update package lists"; exit 1; }
    
    # Use dist-upgrade for better dependency handling
    log "Upgrading system packages..."
    apt dist-upgrade -y || { error "Failed to upgrade packages"; exit 1; }
else
...
apt install -y python3 python3-pip python3-venv python3-dev git curl wget htop \
    build-essential software-properties-common apt-transport-https ca-certificates \
    lsb-release gnupg unzip zip jq net-tools iotop sysstat ntp \
    ufw fail2ban || { error "Failed to install essential packages"; exit 1; }
...
EOF
    netplan apply || { error "Failed to apply netplan configuration"; }
else
...
if [ ! -d "${AI_CLUSTER_DIR}/venv" ]; then
    su - $USERNAME -c "python3.10 -m venv ${AI_CLUSTER_DIR}/venv"
fi

# 7. Install Ray and dependencies
