...
RAY_VERSION="2.9.3"  # Updated to the latest stable version
...
su - $USERNAME -c "source ${AI_CLUSTER_DIR}/venv/bin/activate && \
    ...
    pip install 'ray[default,serve,tune]>=2.0.0' fastapi==0.95.2 uvicorn==0.22.0 \
...
if ! su - $USERNAME -c "source ${AI_CLUSTER_DIR}/venv/bin/activate && python -c 'import ray; print(\"Ray version:\", ray.__version__)'" > /dev/null; then
